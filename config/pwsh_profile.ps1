...
# Lazy PATH refresh flag
$global:_pathRefreshed = $false
...
# ============================================================================
# Oh My Posh - Powerline Prompt Customization (FR-016) - DEFERRED
# ============================================================================
# Initialize Oh My Posh after first prompt (saves ~110ms on startup)

$global:_ompInitialized = $false
function global:_InitOhMyPosh {
    if ($global:_ompInitialized) { return }
...
# Temporary simple prompt until oh-my-posh loads
function global:prompt {
    _InitOhMyPosh
    # After init, oh-my-posh replaces this prompt function
    if ($global:_ompInitialized) {
        # Call the new prompt set by oh-my-posh
...
# ============================================================================
# Chocolatey Profile Module (FR-017) - LAZY LOADED
# ============================================================================
# Import Chocolatey helper functions only when needed (refreshenv, choco)
# Saves ~190ms on profile load

$global:_chocoLoaded = $false
function _LoadChocolatey {
    if ($global:_chocoLoaded) { return }
    $chocolateyProfile = "$env:ChocolateyInstall\helpers\chocolateyProfile.psm1"
    if (Test-Path $chocolateyProfile) {
        Import-Module $chocolateyProfile
        $global:_chocoLoaded = $true
    }
}

# Wrapper for refreshenv
function refreshenv { _LoadChocolatey; ChocolateyProfile\Update-SessionEnvironment @args }

# CommandNotFoundAction - lazy PATH refresh + choco loading
$ExecutionContext.InvokeCommand.CommandNotFoundAction = {
    param($commandName, $eventArgs)

    # First try: refresh PATH from registry
    if (-not $global:_pathRefreshed) {
        $global:_pathRefreshed = $true
        $env:PATH = [Environment]::GetEnvironmentVariable('PATH', 'Machine') + ';' + [Environment]::GetEnvironmentVariable('PATH', 'User')
        $eventArgs.StopSearch = $false
        return
    }

    # Second try: load choco if needed
    if ($commandName -eq 'choco' -and -not $global:_chocoLoaded) {
        _LoadChocolatey
        $eventArgs.StopSearch = $false
    }
}
...
# Alt+g: Open git aliases in fzf and execute selected alias
